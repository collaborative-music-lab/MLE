...
function draw()
{
	...
	with (sketch) {
		...
		// fill arc			
		glcolor(vfrgb);
		v = Math.max(val,0.0)+0.045;			
		slices = Math.max(val*90,10);
		offset = 0.75*2*Math.PI;
		arc = (v*arcLength*2*Math.PI);
		glbegin("tri_strip");
		for (i=0;i<slices;i++) {
			ratio = i/(slices-1);
		...
		// x = 0.5;
		// y = 0.75;
		// var notchrgb = [0,0,0,1];
		// beginstroke("basic2d");
		// strokeparam("order",1)
		// strokeparam("slices",2)
		// strokeparam("color",vfrgb);
		
		// strokeparam("scale",0.15); //thickness of line
		// strokepoint(-0.52,-0.58);
		// strokepoint(-0.,-0.);			
		// endstroke();
		...
		theta = theta * arcLength*0.977 + 0.33;
		var lineLength = 0.7;
